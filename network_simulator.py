...
class NetworkDiagramApp:
    ...
    def calculate_hierarchical_layout(self):
        ...
        try:
            ...
            levels = {}
            node_levels = {}
            for node_idx, node in enumerate(topo_order):
            ...
            pos = {}
            level_width_scale = 5.0 # Increased for more horizontal spread
            node_spacing_scale = 2.5 # Increased for more vertical spread
            max_nodes_in_level = max(len(nodes_in_lvl) for nodes_in_lvl in levels.values()) if levels else 1
            if max_nodes_in_level == 0 : max_nodes_in_level = 1
            ...
            return nx.spring_layout(graph, seed=42, k=2.5/np.sqrt(graph.number_of_nodes()) if graph.number_of_nodes() > 0 else 1.0, iterations=100)

    def draw_network_diagram(self):
        if not st.session_state.activities or not st.session_state.graph.nodes():
            return None

        fig, ax = plt.subplots(figsize=(20, 16)) # Slightly larger figure
        ...
        node_size_regular = 4500 # Increased node size
        node_size_critical = 4800 # Increased critical node size
        arrow_size = 30 # Increased arrow size
        ...
        if not valid_pos and all_graph_nodes:
        ...
        nx.draw_networkx_nodes(graph, valid_pos,
                               ...
                               node_color=regular_node_color, node_size=node_size_regular,
        ...
        if critical_nodes_set:
            nx.draw_networkx_nodes(graph, valid_pos,
                                   ...
                                   node_color=critical_node_color, node_size=node_size_critical,
                                   edgecolors=critical_border_color, linewidths=3.5, ax=ax, alpha=1.0)
            nx.draw_networkx_nodes(graph, valid_pos,
                                   nodelist=[n for n in critical_nodes_set if n in all_graph_nodes and n in valid_pos],
                                   node_color='none', node_size=node_size_critical + 200,
                                   edgecolors=critical_edge_color, linewidths=1.5, ax=ax, alpha=0.6)
        ...
        nx.draw_networkx_edges(graph, valid_pos, edgelist=regular_edges_list,
                               edge_color=regular_edge_color, width=2.5, arrows=True, arrowsize=arrow_size, # Increased width
                               arrowstyle='-|>', node_size=node_size_regular, ax=ax, connectionstyle="arc3,rad=0.05", alpha=0.7)
        if critical_edges_list:
            nx.draw_networkx_edges(graph, valid_pos, edgelist=critical_edges_list,
                                   edge_color=critical_edge_color, width=4.5, style='solid', # Increased width
                                   arrows=True, arrowsize=arrow_size + 5, arrowstyle='-|>',
                                   node_size=node_size_critical, ax=ax, connectionstyle="arc3,rad=0.05", alpha=1.0)
        ...
        for node, label_text in node_labels_custom.items():
            ...
            ax.text(x, y, label_text, ha='center', va='center', fontsize=12, fontweight=font_weight, color=text_color, # Increased fontsize
                    bbox=dict(boxstyle="round,pad=0.3", facecolor='white', edgecolor='none', alpha=0.75)) # Increased pad
        ...
        for u, v, data in graph.edges(data=True):
            ...
            edge_labels_custom[(u,v)] = f"{activity_id} ({duration:.1f}d){star}"
        ...
        for (u,v), label_text in edge_labels_custom.items():
            ...
            offset_dx, offset_dy = -np.sin(angle) * 0.2, np.cos(angle) * 0.2 # Slightly increased offset
            ...
            ax.text(x_mid + offset_dx, y_mid + offset_dy, label_text, ha='center', va='center', fontsize=11, fontweight=font_w, color=text_col, # Increased fontsize
                    bbox=dict(boxstyle="round,pad=0.4", facecolor=bbox_fc, edgecolor='grey', alpha=0.9, lw=1.5 if is_critical_edge else 0.5)) # Increased pad

        ax.set_title("Project Network Diagram & Critical Path", fontsize=24, fontweight='bold', pad=30, color='#333') # Increased fontsize

        if st.session_state.project_duration > 0:
            ax.text(0.5, 0.96, f"Total Project Duration: {st.session_state.project_duration:.1f} days",
                    transform=ax.transAxes, ha='center', fontsize=16, color=critical_edge_color, weight='bold') # Increased fontsize

        ax.axis('off')
        plt.margins(0.15) # Increased margin

        from matplotlib.lines import Line2D
        legend_elements = [
            Line2D([0], [0], marker='o', color='w', label='Regular Node (ID, ES/LF)',
                   markerfacecolor=regular_node_color, markeredgecolor=node_border_color, markersize=18, mew=2.5), # Increased markersize
            Line2D([0], [0], marker='o', color='w', label='Critical Node (ID, ES/LF)',
                   markerfacecolor=critical_node_color, markeredgecolor=critical_border_color, markersize=18, mew=3), # Increased markersize
            Line2D([0], [0], color=regular_edge_color, lw=3, label='Regular Activity (Name (Duration))'), # Increased lw
            Line2D([0], [0], color=critical_edge_color, lw=4, label='Critical Activity (Name (Duration) ⭐)'), # Increased lw
        ]
        ax.legend(handles=legend_elements, loc='lower left', fontsize=13, frameon=True, fancybox=True, shadow=True, title="Legend", title_fontsize=15) # Increased fontsize

        if st.session_state.critical_path_nodes:
            critical_nodes_str = ", ".join(map(str, st.session_state.critical_path_nodes))
            if len(critical_nodes_str) > 60: # Adjusted length for truncation
                 critical_nodes_str = critical_nodes_str[:57] + "..."
            ax.text(0.98, 0.02, f"Critical Nodes: {critical_nodes_str}",
                    transform=ax.transAxes, ha='right', va='bottom', fontsize=12, # Increased fontsize
                    bbox=dict(boxstyle="round,pad=0.5", facecolor='#ffebee', edgecolor=critical_border_color, alpha=0.85), # Increased pad
                    color=critical_border_color, weight='bold')

        plt.tight_layout(pad=2.0) # Increased pad
        return fig

    def add_dummy_data(self):
        self.clear_all_activities()

        # Sample data from a common CPM example structure
        dummy_activities = [
            ("A", "1", "2", 5.0),
            ("B", "1", "3", 7.0),
            ("C", "2", "4", 6.0),
        ...
        # Expected critical path for this data: B -> D -> F (7+4+9 = 20) or B -> E -> G (7+3+8 = 18)
        # Path A->C->F = 5+6+9 = 20.
        # Critical path: A-C-F and B-D-F (Duration: 20)

        temp_activities_list = st.session_state.activities
        temp_graph_obj = st.session_state.graph
        ...
        try:
            ...
            st.session_state.project_duration = 0
            st.session_state.analysis_results = {}
            return True, "Sample project data (A, B, C...) loaded. Previous data was cleared."
        except ValueError as ve:
             st.session_state.activities = temp_activities_list
             st.session_state.graph = temp_graph_obj
             return False, str(ve)
        except Exception as e:
             st.session_state.activities = temp_activities_list
             st.session_state.graph = temp_graph_obj
...
def main():
    ...
    fig_generated = None # Initialize fig_generated
    if st.session_state.activities:
        fig_generated = app.draw_network_diagram()
        if fig_generated:
            st.pyplot(fig_generated, use_container_width=True)
    ...
    if analysis_results_exist:
        ...
        if not df_display.empty:
            ...
            if fig_generated: # Use the stored figure object
                img_buffer = io.BytesIO()
                fig_generated.savefig(img_buffer, format='png', dpi=300, bbox_inches='tight', facecolor='white')
                img_buffer.seek(0)
                ...
                dl_cols[0].markdown("<small>Diagram not generated or available for download.</small>", unsafe_allow_html=True)
